...
// phep cong tren duong cong elliptic
plus =  (Px, Py, Qx, Qy, c) => {
    if (!(Px && Py && Qx && Qy)) {
        return 'Moi nhap toa do cua P va Q';
    }
    let P = c.getPoint(parseInt(Px), parseInt(Py));
    let Q = c.getPoint(parseInt(Qx), parseInt(Qy));
    if (typeof P === 'undefined') {
        return 'P khong thuoc tren duong Elliptic';
    }
    if (typeof Q === 'undefined') {
        return 'P khong thuoc tren duong Elliptic';
    }
    return (`P(${Px},${Py}) + Q(${Qx},${Qy}) = ${P.plus(Q).toString()}`);
}

// phep nhan tren duong cong elliptic
multiply =  (Px, Py, n, c) => {
    if (!(Px && Py && n)) {
        return 'Moi nhap toa do P và n';
    }
    let P = c.getPoint(parseInt(Px), parseInt(Py));
    ...
    return (`${n}P(${Px},${Py}) = ${_result?.toString()}`);
}
